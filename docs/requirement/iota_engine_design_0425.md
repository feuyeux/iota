...
✅ **已实现核心功能：**
- Visibility 基础设施（Collector/Store/Types）
- ContextManifest/MemoryVisibilityRecord/TokenLedger/LinkVisibilityRecord
- TraceSpan 覆盖关键链路
- Engine API (getExecutionVisibility/listSessionVisibility)
- Agent HTTP API (execution/session visibility + app-snapshot)
- CLI 基础查询命令
- Redis + Local fallback 存储
- 四后端基础 adapter mapping

🔄 **部分实现：**
- CLI 高级命令（会话级/交互式/导出）未实现
- WebSocket visibility delta 订阅需补充测试
- 四后端真实样本集成测试不足

❌ **未实现：**
- CLI 架构组件拆分
- OpenTelemetry exporter 预留接口
- Visibility full-content 短 TTL 策略
...
❌ **仍未实现：**
- 独立的 CLI 架构组件拆分：VisibilityClient, VisibilityFormatter, VisibilityMonitor, VisibilityExporter 仍以内聚命令模块形式存在，尚未拆为独立服务类；当前 `iota interactive` 也仍是本地 CLI 直连 Engine，而不是经 Agent WebSocket
...
```text
iota:visibility:context:<executionId>       String (JSON)
iota:visibility:memory:<executionId>        String (JSON)
iota:visibility:tokens:<executionId>        String (JSON)
iota:visibility:link:<executionId>          String (JSON)
iota:visibility:spans:<executionId>         List (JSON items)
iota:visibility:<executionId>:chain         Hash (spanId → TraceSpan JSON)
iota:visibility:mapping:<executionId>       List (JSON items)
...
class VisibilityClient {
  ...
  async subscribeInteractive(
    ...
    if (!this.agentUrl) {
      throw new Error('Visibility subscribeInteractive requires agent WebSocket; current iota interactive TUI does not use this path');
    }
