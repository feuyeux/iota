...
内容：
分别展示4种backend的cli示例 并总结完整的流程原理图--以组件为中心 流程在组件间的箭头线上标明 序号和描述 (文字版)
...
Hermes 使用 MCP (Model Context Protocol) 方式，LLM 直接调用 fun.* 工具：
...
└────────────────────────────┬────────────────────────────────────────┘
                             ...
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│ 5. Hermes LLM 并发发起 MCP 工具调用                                   │
│    LLM → Hermes Agent → MCP call fun.cpp                            │
│    LLM → Hermes Agent → MCP call fun.typescript                     │
│    LLM → Hermes Agent → MCP call fun.rust                           │
│    ... (7 个并发调用)                                                 │
└────────────────────────────┬────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│ 6. iota-fun-mcp server 处理每个工具调用                               │
│    MCP call fun.cpp → iota-fun-mcp server                           │
│      → IotaFunEngine.execute({language: "cpp"})                     │
│      → spawn g++ → 编译 → 执行 → 返回 "奔跑"                         │
│      → 返回给 Hermes Agent → 返回给 LLM                              │
│                                                                       │
│    MCP call fun.typescript → iota-fun-mcp server                    │
│      → IotaFunEngine.execute({language: "typescript"})              │
...
Hermes LLM → MCP call fun.cpp → iota-fun-mcp server → IotaFunEngine.execute() → 返回给 Hermes Agent → 返回给 LLM → LLM 组合结果
