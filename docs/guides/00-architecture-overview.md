...
- Claude Code、Codex、Gemini CLI、Hermes 的原生输出必须先映射为 `RuntimeEvent`。
...
![Iota 架构总览](../diagrams/00-current-architecture.svg)
...
| Backend | 文件 | 进程模型 | 协议 | 说明 |
|---|---|---|---|---|
| Claude Code | `iota-engine/src/backend/claude-code.ts` | 每次执行子进程 | stream-json NDJSON over stdio | 映射 thinking/output/tool/approval 扩展事件 |
| Codex | `iota-engine/src/backend/codex.ts` | 每次执行子进程 | NDJSON over stdio | 映射 Codex exec 事件 |
| Gemini CLI | `iota-engine/src/backend/gemini.ts` | 每次执行子进程 | stream-json NDJSON over stdio | 支持 message/tool/result 映射 |
| Hermes Agent | `iota-engine/src/backend/hermes.ts` | 长运行子进程 | ACP JSON-RPC 2.0 over stdio | 会写隔离 Hermes runtime 配置并复用进程 |
...
![Iota 执行与 App 读取模型](../diagrams/00-execution-read-model-flow.svg)
...
![Engine 内部结构](../diagrams/00-engine-internals.svg)
...
| Backend | 命令模型 | 输入输出 |
|---|---|---|
| Claude Code | `claude --print --output-format stream-json ...` | prompt 作为 CLI 参数/stdio，stdout 为 stream-json NDJSON |
| Codex | `codex exec ...` | stdout 为 NDJSON |
| Gemini CLI | `gemini --output-format stream-json --prompt ...` | stdout 为 stream-json NDJSON |
| Hermes | `hermes acp` | ACP JSON-RPC 2.0 over stdio，长运行进程 |
...
| Memory index | `iota:memories:{type}:{scopeId}` | Sorted Set | 按类型/scope 查询 |
...
| Unified Memory | Redis | episodic/procedural/factual/strategic，带 TTL/index |
...
| 能力 | 状态 | 说明 |
|---|---|---|
| Claude/Codex/Gemini adapter | 稳定 | 每执行子进程，输出映射为 RuntimeEvent |
| Hermes adapter | 集成中 | ACP JSON-RPC，长运行进程，对本地 Hermes 配置敏感 |
