...
- 至少安装并可访问一个 Backend 后端（Claude Code、Codex、Gemini CLI、Hermes Agent 或 OpenCode；ACP adapter-backed Claude/Codex 还需要对应 shim 可执行入口）
...
graph LR
    ...
    Engine -->|启动| Backend[Backend 后端子进程<br/>Claude/Codex/Gemini/Hermes/OpenCode]
    Backend -->|stdout ACP JSON-RPC / legacy NDJSON| Engine
    Engine -->|事件| CLI
    CLI -->|输出| User
```
...
- **Engine 引擎 → Backend 后端**: 子进程 stdio 管道 — 首选 ACP JSON-RPC 2.0；Claude/Codex/Gemini 保留 legacy NDJSON fallback
- **Backend 后端 → Engine 引擎**: stdout/stderr 管道发出 ACP JSON-RPC notification/response 或 legacy NDJSON 事件
...
# 统一发现 claude / codex / gemini / hermes / opencode；Windows 可在 WSL/Git Bash 中运行
```
...
| Hermes Agent | `iota:config:backend:hermes` | `protocol=acp`, `env.HERMES_API_KEY`, `env.HERMES_BASE_URL=https://api.minimaxi.com/anthropic`, `env.HERMES_MODEL=MiniMax-M2.7`, `env.HERMES_PROVIDER=minimax-cn` | Redis 分布式配置；Iota 将这些转换为隔离的 Hermes 运行时配置 |
| OpenCode | `iota:config:backend:opencode` | `protocol=acp`，以及 OpenCode 所需的 env/model 字段 | 原生 ACP：`opencode acp` |
...
iota config set acpAdapterArgs "--experimental" --scope backend --scope-id gemini  # 可选额外参数

# Claude/Codex adapter-backed ACP
...
iota config set acpAdapter "@openai/codex-acp" --scope backend --scope-id codex
```

默认配置下 Claude/Codex/Gemini 仍可使用 legacy native adapter；设置 `protocol=acp` 后会优先走 ACP，早期启动/握手失败会回退到 native fallback。
...
**选项**:
- `--backend <backend>` — 使用的 Backend 后端（`claude-code`、`codex`、`gemini`、`hermes`、`opencode`）
...
```json
{
  ...
  "opencode": { "healthy": false, "status": "degraded", "lastError": "Executable not found" }
}
```
