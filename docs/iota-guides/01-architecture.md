...
- Backend 统一使用 ACP JSON-RPC 2.0；不再保留非 ACP 降级路径
...
flowchart TB
  ...
  subgraph L5["Layer 5 - Backend Protocol Adapters"]
    Claude["Claude Code<br/>ACP JSON-RPC 2.0"]
    Codex["Codex<br/>ACP JSON-RPC 2.0"]
    Gemini["Gemini CLI<br/>ACP JSON-RPC 2.0"]
...
| Backend ACP-only adapters | 已接入，需按 backend 跑真实 traced request 验收 |
